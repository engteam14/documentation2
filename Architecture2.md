...
#### Physics
To implement physics in our game we used Libgdx’s `Box2D`. We implemented this using `Rigidbody` components and a `PhysicsManager`. The `Rigidbody` component is used to define the collision fixtures of the object and is a component of `CannonBall`, `PowerUpPickup`, `Buidling`, `Ship` and `Obstacle`.
...
#### Saving
The `SaveManager` class is the class that enables UR_SAVE_GAME. This creates and also loads a LibGdx preferences file that saves the state of all `ship` objects including the plater and also all `College` objects. Again this came under the Managers heading in the abstract diagram but it wasn't until we chose the game engine that we could decide exactly how this would be implemented.
